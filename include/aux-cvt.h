...
public:
    ...
    {
        T::push((const unsigned char*)str,strlen(str));
        return *this;
    };

    ostream_t& operator << (char c)
    {
        T::push((unsigned char)c);
        return *this;
    };
...
public:
    ...
    pod::wchar_buffer& get_buffer()
    {
        return buffer;
    }

    operator aux::wchars() const
    {
        return aux::wchars(buffer.data(),buffer.length());
    }
    aux::wchars chars() const
    {
        return aux::wchars(buffer.data(),buffer.length());
    }
...
public:
    itot(int n, int radix = 10)
    {
        ...
        while( n /= radix );   // conversion. number is reversed.
        if(sign<0) *wstr++ ='-';
        *wstr = 0;
        // reverse the string
