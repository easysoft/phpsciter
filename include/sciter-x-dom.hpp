...
protected:
    void use(HNODE h)
    {
        hn = ( SciterNodeAddRef(h) == SCDOM_OK)? h: 0;
    }
    void unuse()
    {
        if(hn) SciterNodeRelease(hn);
        hn = 0;
    }
    void set(HNODE h)
    {
...
public:
    ...
    HNODE operator[](int idx) const
    {
        return this->operator[](unsigned(idx));
    }
    ...
    void text(const sciter::string& s)
    {
        text(s.c_str(),UINT(s.length()));
    }
...
protected:
    ...
    void use(HELEMENT h)
    {
        he = ( Sciter_UseElement(h) == SCDOM_OK)? h: 0;
    }
    void unuse()
    {
        if(he) Sciter_UnuseElement(he);
        he = 0;
    }
    void set(HELEMENT h)
    {
...
public:
    ...
    bool operator == (HELEMENT rs ) const
    {
        return he == rs;
    }
