...
public:
    archive():har(0) {}
    // open archive blob:
    bool open( LPCBYTE data, UINT data_length )
    {
        close();
        har = SAPI()->SciterOpenArchive(data,data_length);
        return har != 0;
    }
    bool open( aux::bytes data )
    {
        return open( data.start, data.length );
    }
    void close()
    {
        if(har) SAPI()->SciterCloseArchive(har);
        har = 0;
    }
    // get archive item:
    aux::bytes get( LPCWSTR path )
    {
        ...
        if( aux::chars_of(path).like(WSTR("//*")))
            path += 2;
        SAPI()->SciterGetArchiveItem(har,path,&pb,&blen);
...
struct host
{
    ...
    LRESULT handle_notification(LPSCITER_CALLBACK_NOTIFICATION pnm)
    {
        ...
        {
        case SC_LOAD_DATA:
            return static_cast<BASE*>(this)->on_load_data((LPSCN_LOAD_DATA) pnm);
        case SC_DATA_LOADED:
            return static_cast<BASE*>(this)->on_data_loaded((LPSCN_DATA_LOADED)pnm);
        //case SC_DOCUMENT_COMPLETE: return on_document_complete();
        case SC_ATTACH_BEHAVIOR:
            return static_cast<BASE*>(this)->on_attach_behavior((LPSCN_ATTACH_BEHAVIOR)pnm );
        case SC_ENGINE_DESTROYED:
            return static_cast<BASE*>(this)->on_engine_destroyed();
        case SC_POSTED_NOTIFICATION:
            return static_cast<BASE*>(this)->on_posted_notification((LPSCN_POSTED_NOTIFICATION)pnm);
        case SC_GRAPHICS_CRITICAL_FAILURE:
            static_cast<BASE*>(this)->on_graphics_critical_failure();
            return 0;
        }
        return 0;
    }
...
//        }
    ...
    virtual LRESULT on_posted_notification( LPSCN_POSTED_NOTIFICATION lpab )
    {
        return 0;
    }
