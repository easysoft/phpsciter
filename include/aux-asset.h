...
public:
    ...
    asset_ptr(const asset_ptr<R>& cp):p(0)
    {
        if (cp.p) (p = cp.p)->add_ref();
    }
    ...
    R* operator=(R* lp)
    {
        if(p != lp) return assign(p, lp);
        return p;
    }
    R* operator=(const asset_ptr<R>& lp)
    {
        if(p != lp) return assign(p, lp.p);
        return p;
    }
...
public:
    ...
    ptr(const ptr<T>& cp):p(0)
    {
        if (cp.p) (p = cp.p)->AddRef();
    }
