...
public:
    ...
    virtual const WCHAR*    stream_name()
    {
        static WCHAR z = 0;
        return &z;
    }
...
class string_istream: public stream
{
    ...
    const WCHAR* _end;
public:
    string_istream(const WCHAR* str, unsigned length = 0): _pos(str),_end(str)
    {
        if(length == 0) length = (unsigned int)aux::wcslen(str);
        _end = _pos + length;
    }
    virtual int get()
    {
        return (*_pos && _pos < _end)? *_pos++ : -1;
    }
};
...
public:
    string_ostream()
    {
        _p = _str = (WCHAR*)malloc( 128 * sizeof(WCHAR) );
        _end = _str + 128;
    }
...
public:
    ...
    {
        int c = getchar();
        return c != EOF? oem2wchar(char(c)) : -1;
    }
    virtual bool put(int v)
    {
