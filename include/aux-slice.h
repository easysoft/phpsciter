...
{
    return isspace(c & 0xff) != 0;
}
inline bool is_space( WCHAR c )
{
    return iswspace(c) != 0;
}
...
{
    return isdigit(c & 0xff) != 0;
}
inline bool is_digit( WCHAR c )
{
    return iswdigit(c) != 0;
}
...
{
    return isalpha(c & 0xff) != 0;
}
inline bool is_alpha( WCHAR c )
{
    return iswalpha(c) != 0;
}
...
{
    return isalnum(c & 0xff) != 0;
}
inline bool is_alnum( WCHAR c )
{
    return iswalnum(c) != 0;
}

template<typename CT>
inline size_t wcslen( const CT* s )
{
    const CT *p = s;
    while (*p) p++;
    return p - s;
...
inline slice<T> make_slice(const T(&arr)[size])
{
...
{
    return t? wchars(t,(unsigned int)wcslen(t)):wchars();
}
inline chars   chars_of( const char *t )
{
    return t? chars(t,(unsigned int)strlen(t)):chars();
}
...
template<typename T>
slice<T> elements_of(const T* s, size_t l)
{
    return slice<T>(s, l);
}

template<typename T>
slice<T> elements_of( const std::vector<T> &s )
{
    return slice<T>(s.data(), s.size());
}

template<typename T, int size>
slice<T> elements_of(T (& arr)[size])
{
    return slice<T>(&arr[0],size);
}

template<typename T, int size>
slice<T> elements_of(const T (& arr)[size])
{
    return slice<T>(&arr[0],size);
}

template<typename T>
std::basic_string<T> make_string( aux::slice<T> s )
{
    return std::basic_string<T>(s.start, s.length);
}
...
class tokens
{
    ...
    {
        for(const T* t = delimeters; t && *t; ++t) if(el == *t) return true;
        return false;
    }
    const T*    tok()
    {
        for(; p < tail; ++p) if(is_delimeter(*p)) return p++;
        return p;
    }
public:
...
private:
    void set ( CT from, CT to, bool v )
    {
        for ( unsigned i = charcode(from); i <= charcode(to); ++i )
        {
            ...
            if( v ) codes[octet] |= 1 << bit;
            else codes[octet] &= ~(1 << bit);
        }
    }
    void init ( unsigned char v )
    {
        memset(codes,v,(SET_SIZE >> 3));
    }
public:
