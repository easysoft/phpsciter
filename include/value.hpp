...
public:
    ...
    {
        ValueInit(this);
        ValueIntDataSet(this, v?1:0, T_BOOL, 0);
    }
    value( int  v )
    {
        ValueInit(this);
        ValueIntDataSet(this, v, T_INT, 0);
    }
    value( double v )
    {
        ValueInit(this);
        ValueFloatDataSet(this, v, T_FLOAT, 0);
    }
    value( float v )
    {
        ValueInit(this);
    ...
    {
        ValueInit(this);
        ValueStringDataSet(this, LPCWSTR(s), (slen || !s)? slen : (unsigned int)str_length(s), 0);
    }
    value( const string& s )
    {
        ValueInit(this);
        ValueStringDataSet(this, LPCWSTR(s.c_str()), UINT(s.length()), 0);
    }
    value( const astring& s )
    {
        aux::a2w as(s.c_str());
        ValueInit(this);
        ValueStringDataSet(this, LPCWSTR(as.c_str()), UINT(as.length()), UT_STRING_SYMBOL);
    }
    value( aux::wchars ws )
    {
        ValueInit(this);
        ValueStringDataSet(this, LPCWSTR(ws.start), ws.length, 0);
    }

    value( aux::bytes bs )
    {
        ValueInit(this);
        ValueBinaryDataSet(this, bs.start, bs.length, T_BYTES, 0);
    }

    value( const value* arr, unsigned n )
    {
        ValueInit(this);
        for( unsigned i = 0; i < n; ++i ) set_item(int(i),arr[i]);
    }
#ifdef CPP11
    ...
    {
        value t;
        ValueInt64DataSet(&t, v, T_CURRENCY, 0);
        return t;
    }
    static value date( INT64 v, bool is_utc = true /* true if ft is UTC*/ )
    {
        value t;
        ValueInt64DataSet(&t, v, T_DATE, is_utc);
        return t;
    }
#ifdef WIN32
    static value date( FILETIME ft, bool is_utc = true /* true if ft is UTC*/ )
    {
        value t;
        ValueInt64DataSet(&t, *((INT64*)&ft), T_DATE, is_utc);
        return t;
    }
#endif
    static value symbol( aux::wchars wc )
    {
        value t;
        ValueInit(&t);
        ValueStringDataSet(&t, LPCWSTR(wc.start), wc.length , 0xFFFF);
        return t;
    }

    /** set color value, abgr - a << 24 | b << 16 | g << 8 | r, where a,b,g,r are bytes */
    static value color(UINT abgr)
    {
        value t;
        ValueInit(&t);
        ValueIntDataSet(&t, abgr, T_COLOR, 0);
        return t;
    }

    /** set duration value, seconds */
    static value duration(double seconds)
    {
        value t;
        ValueInit(&t);
        ValueFloatDataSet(&t, seconds, T_DURATION, 0);
        return t;
    }

    /** set angle value, radians */
    static value angle(double radians)
    {
        value t;
        ValueInit(&t);
        ValueFloatDataSet(&t, radians, T_ANGLE, 0);
        return t;
    }
    ...
    {
        return t == T_STRING && u == UT_STRING_SYMBOL;
    }
    bool is_error_string() const
    {
        return t == T_STRING && u == UT_STRING_ERROR;
    }
    bool is_date() const
    {
        return t == T_DATE;
    ...
    void* get_object_data() const
    {
        LPCBYTE pv = 0;
        unsigned int dummy;
        UINT r = ValueBinaryData(this,&pv,&dummy);
        r = r;
        assert(r == HV_OK);
        return (void*)pv;
    }
...
//#if defined(HAS_TISCRIPT)
    ...
    {
        return t == T_OBJECT && u == UT_OBJECT_NATIVE;
    }
    bool is_object_array() const
    {
        return t == T_OBJECT && u == UT_OBJECT_ARRAY;
    }
    bool is_object_function() const
    {
        return t == T_OBJECT && u == UT_OBJECT_FUNCTION;
    }
    bool is_object_object() const
    {
        return t == T_OBJECT && u == UT_OBJECT_OBJECT;    // that is plain TS object
    }
    bool is_object_class() const
    {
        return t == T_OBJECT && u == UT_OBJECT_CLASS;    // that is TS class
    }
    bool is_object_error() const
    {
        return t == T_OBJECT && u == UT_OBJECT_ERROR;    // that is TS error
    }
...
template<> inline unsigned value::get<unsigned>() const
{
    return (unsigned)get(0);
}
...
template<> inline float value::get<float>() const
{
    return (float)get(0.0);
}
template<> inline string value::get<string>() const
{
    return to_string();
}
//template<> inline const value& value::get<const value&>() const  { return *this; }
template<> inline value value::get<value>() const
{
    return *this;
}
...
public:
    ~value_key_a() {}
    value_key_a& operator= (const value& val)
    {
        col.set_item(key,val);
        return *this;
    }
};
...
public:
    ~value_idx_a() {}
    value_idx_a& operator= (const value& val)
    {
        col.set_item(idx,val);
        return *this;
    }
};
...
{
public:
    native_function(const native_function_t& f): func(f)
    {
        assert(f);
    }
